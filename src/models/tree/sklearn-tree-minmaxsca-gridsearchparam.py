...
train_x = utils.minmax_scaler_oh(utils.one_hot_encode(df = train_raw.drop(['Transported', 'PassengerId'], axis=1)))
train_y = train_raw.Transported


# GridSerach para mejores parametros
grid_parameters = {'criterion':['gini', 'entropy', 'log_loss'], 'splitter':['best','random'], 'max_depth':list(range(10,25)), 'min_samples_split':list(range(45,65)), 
                   'min_samples_leaf':list(range(5,25)), 'max_features':['None', 'sqrt', 'log2']}
...
dtc_gs.best_score_
# mejor resultado con 0.7832793672208774
dtc_gs.best_params_
#{'criterion': 'gini',
# 'max_depth': 14,
# 'max_features': 'sqrt',
...
# 'splitter': 'best'}
# El escalado no ha afectado a los parámetros, y con este escalado se ha obtenido un 
# resultado más malo que con el dataset sin escalar
...
utils.generate_submission(labels = true_labels, method = "tree", notes = "minmaxsca_gridSearch_parameters")
