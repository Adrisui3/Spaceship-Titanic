...
train_x = utils.robust_scaler_oh(utils.one_hot_encode(df = train_raw.drop(['Transported', 'PassengerId'], axis=1)))
...
# 'splitter': 'best'}
# Mismos parámetros, pero ha mejorado la precisión ligeramente

best_parameters = dtc_gs.best_params_
# Cargamos conjunto de test
test_raw = utils.load_test()
test = utils.robust_scaler_oh(utils.one_hot_encode(df = test_raw.drop(["PassengerId"], axis = 1)))
...
utils.generate_submission(labels = true_labels, method = "tree", notes = "robustsca_gridSearch_parameters")
