...
train_x = utils.standard_scaler_oh(utils.one_hot_encode(df = train_raw.drop(['Transported', 'PassengerId'], axis=1)))
...
dtc_gs.best_score_
# mejor resultado con 0.784889620782244
dtc_gs.best_params_
# 'ccp_alpha': 0.0002,
# 'criterion': 'gini',
# 'max_depth': 16,
...
# 'splitter': 'best'}
# Mismos parámetros y misma precisión que robust scaler

best_parameters = dtc_gs.best_params_
# Cargamos conjunto de test
test_raw = utils.load_test()
test = utils.standard_scaler_oh(utils.one_hot_encode(df = test_raw.drop(["PassengerId"], axis = 1)))
...
utils.generate_submission(labels = true_labels, method = "tree", notes = "standardsca_gridSearch_parameters")
