...
train_X = train_X.drop(["Cabin_deck_T", "SM_FC", "VIP_1.0"], axis=1)
...
cv = cross_validate(estimator = LogisticRegression(random_state = 1234, max_iter=1, solver = "newton-cholesky", C = 7, tol = 0.0001, fit_intercept=True, class_weight=None), X = train_X, y = train_y, cv = 10, n_jobs = -1, verbose = 5)
...
test = test.drop(["Cabin_deck_T", "SM_FC", "VIP_1.0"], axis=1)
...
# log = LogisticRegression(verbose=True, random_state = 1234, max_iter=1, solver = "newton-cholesky", C = 2, tol = 0.0001, fit_intercept=False, class_weight="balanced").fit(X = train_X, y = train_y)
log = LogisticRegression(verbose=True, random_state = 1234, max_iter=1, solver = "newton-cholesky", C = 7, tol = 0.0001, fit_intercept=True, class_weight=None).fit(X = train_X, y = train_y)
...
utils.generate_submission(labels = pred_labels, method = "log", notes = "no-outliers-feat-selection2")
