...
train_X = train_X.drop(["VIP_1.0", "Cabin_deck_T"], axis=1)
...
cv = cross_validate(estimator = LogisticRegression(random_state = 1234, max_iter=1, solver = "newton-cholesky", C = 2, tol = 0.0001, fit_intercept=False, class_weight="balanced"), X = train_X, y = train_y, cv = 10, n_jobs = -1, verbose = 5)
...
test = test.drop(["VIP_1.0", "Cabin_deck_T"], axis=1)
...
utils.generate_submission(labels = pred_labels, method = "log", notes = "feat_selection")
